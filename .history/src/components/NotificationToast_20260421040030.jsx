    ...
    <div className="fade-in fixed bottom-6 left-1/2 -translate-x-1/2 z-[300] w-full max-w-sm px-4">
      <div className={`flex items-center gap-3 px-5 py-4 rounded-2xl shadow-xl border transition-all duration-300
        ...
        <div className={`flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center
          ${type === 'success' ? 'bg-green-500/15' : 'bg-red-50'}`}>
          {type === 'success' ? (
            <CheckCircle size={22} />
          ) : (
            <AlertCircle size={22} />
        ...
        <button 
          ...
          className="flex-shrink-0 p-1.5 -mr-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors"
        >
          <X size={18} />
        </button>
      </div>
