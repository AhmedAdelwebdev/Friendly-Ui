  ...
  const sendTelegramNotification = async (data) => {
    try {
      ...
      const message = (isAttempt ? `⭐ <b>Payment Attempt</b>` : `😎 <b>New Order Received</b>`) +
        ...
        `\nzn• Price: $${Number(activeItem.price)} (${priceEGP} EGP)` +
              ...
              <>
                ...
                {step === 2 && (
                  <div className="animate-in fade-in slide-in-from-right-10 duration-500">
                    ...
                    {isBackendDown ? (
                      <div className="p-6 mb-6 text-center animate-in zoom-in-95">
                        <div className="size-16 bg-red-500/10 text-red-500 rounded-3xl flex items-center justify-center mx-auto mb-4">
                          <X size={26} />
                        </div>
                        <h4 className="text-red-500 mb-2">
                          {txIsAr('Service Temporarily Unavailable', 'الخدمة غير متوفرة حالياً')}
                        </h4>
                        <p className="text-text text-base mb-6 sm:px-6">
                          {txIsAr(
                            'We are currently experiencing technical issues with our database. To ensure your payment is processed safely, please complete your order directly via Telegram.',
                            'نواجه حالياً مشكلة تقنية في الموقع. لضمان إتمام طلبك بأمان، يرجى إتمام الطلب مباشرة عبر تليجرام.'
                          )}
                        </p>
                        <Link href="https://t.me/Friendly_Ui" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full bg-primary text-white py-4 rounded-xl text-sm font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all" >
                          <MessageSquare size={18} />
                        ...
                        >
                          {txIsAr('Close and return', 'إغلاق والعودة')}
                        </button>
                      </div>
                    ) : (
