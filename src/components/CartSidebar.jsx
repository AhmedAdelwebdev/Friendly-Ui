  ...
  return (
    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="relative w-14 h-14 bg-gray-50 rounded-xl overflow-hidden border border-gray-100 flex-shrink-0">
          <Image
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`text-[10px] px-2 py-1 rounded-md font-bold ${order.status === 'completed'
          ...
          <div className="flex items-center flex-wrap gap-2 mt-2 text-[11px] text-gray-500">
            ...
            {order.purchaseDate && (
              <div className="flex items-center gap-1">
                <Clock size={12} />
                <span>
                  {new Date(order.purchaseDate).toLocaleString(undefined, {
                    month: 'short',
                    day: 'numeric',
                    hour: '2-digit',
                    minute: '2-digit'
            ...
            {/* ID Button بدل عرض الرقم */}
            <button
              onClick={handleCopy}
              className="px-2.5 py-1 bg-gray-100 rounded-md text-gray-600 hover:bg-primary/10 hover:text-primary transition-all text-[10px] font-bold"
            >
              {copied ? 'Copied!' : 'ID'}
            </button>
      ...
      <div className="mt-4 flex gap-2">
        ...
        {order.status === 'completed' ? (
          <Link
            ...
            target="_blank"
            className="flex-1 bg-primary text-white py-2.5 rounded-xl text-xs flex items-center justify-center gap-2 hover:opacity-90 transition-all"
          >
            <Download size={14} /> Download
          </Link>
        ) : (
          <div className="flex-1 bg-gray-100 text-gray-400 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2">
        ...
        {stage === 'confirm' ? (
          <button
            ...
            className="px-4 py-2.5 bg-red-500 text-white rounded-xl text-xs font-bold hover:bg-red-600 transition-all"
          >
            Confirm
          </button>
        ) : (
...
export const CartSidebar = () => {
  ...
  return (
    <div className="fixed inset-0 z-[150] overflow-hidden">
      ...
      {/* Sidebar */}
      <div className="bg-white absolute right-0 top-0 h-full w-full max-w-sm shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col font-baloo">
          ...
          {orders.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center gap-6 py-20 px-6">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm font-medium leading-relaxed">Your purchased designs and products will appear in this library.</p>
              </div>
              <button
                onClick={() => setIsOrdersOpen(false)}
        ...
        {orders.length > 0 && (
          <div className="p-4 border-t border-border bg-gray-50 text-center">
            <p className="text-[10px] text-gray-400 font-medium">
              Need help with your orders? <Link href="https://t.me/Friendly_Ui" className="text-primary ">Contact Telegram Support</Link>
            </p>
          </div>
        )}
      </div>
