...
import { X, CheckCircle, AlertCircle, Upload, CreditCard, Download, ShieldCheck, ChevronRight, Copy, Check, ArrowRight, MessageSquare, Send, Globe, Loader2 } from 'lucide-react';
...
export const PaymentModal = ({ host, protocol }) => {
  ...
  const handlePayPalSuccess = async (details) => {
    ...
    try {
      ...
      await fetch('/api/delivery', {
        ...
        body: JSON.stringify({
          telegramId,
          order: { name: userInfo.name, productName: activeItem.title },
          fileLink: activeItem.fileLink
        })
      });
          ...
          <div className="flex items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-full transition-all border border-black/5 text-[10px] sm:text-xs text-gray-700 flex items-center gap-1.5 font-bold uppercase tracking-wider"
              >
                <Globe size={14} />
                {lang === 'en' ? 'عربي' : 'EN'}
              </button>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          className="w-full px-5 py-4 bg-gray-50/50 border-2 border-primary/20 rounded-[1.2rem] focus:bg-white focus:border-primary outline-none transition-all text-sm text-gray-900 shadow-sm"
                      ...
                      <button onClick={() => startPaymentAttempt('paypal')} className="group p-4 border-2 border-gray-100 dark:border-white/5 rounded-xl hover:border-primary/30 dark:hover:border-primary! hover:bg-gray-50 dark:hover:bg-primary/20 transition-all text-left flex items-center justify-between rtl:text-right">
                        <div className="flex items-center gap-4 w-full">
                          <div className="w-10 h-10 bg-white! dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-lg flex items-center justify-center transition-transform shadow-sm">
                            <Image src="/paypal.svg" className="size-full" width={40} height={40} alt=""></Image>
                          </div>
                          <div>
