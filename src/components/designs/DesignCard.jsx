...
export const DesignCard = ({ item, index, related }) => {
  ...
  return (
    <div 
      className={`reveal fade-up group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md flex flex-col h-full border border-gray-100 ${related ? '' : ''}`}
    ...
    >
      <Link href={`/designs/${item.id}`} className="aspect-square relative bg-white overflow-hidden block">
        ...
        <div className="absolute top-3 left-3 right-3 flex justify-between items-center">
          ...
          {/* Category */}
          <span className="bg-primary/10 backdrop-blur-sm px-3 py-1 rounded-lg text-[10px] font-medium text-primary">
            {item.category}
          </span>

          {/* Price */}
          {isProduct && (
            <span className="bg-primary/10 backdrop-blur-sm text-primary px-3 py-1 rounded-lg text-xs">
              ${Number(item.price)}
            </span>
          )}
