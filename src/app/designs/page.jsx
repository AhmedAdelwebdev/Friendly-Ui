...
export default function DesignsPage() {
  ...
  const [showFilters, setShowFilters] = useState(false); // 👈 مهم
  ...
  return (
    <div className="py-10 md:py-16">
      ...
      <div className="w-full">
        ...
        {/* Search + Filters */}
        <div ref={topRef} className="w-[90vw] sm:max-w-[80vw] md:max-w-3xl mx-auto p-2 rounded-3xl sm:rounded-full shadow-sm mb-10 bg-primary">
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="relative grow">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 text-lg bg-body border border-primary/20 rounded-full outline-none"
              />
            ...
            {/* Types (Desktop) */}
            <div className="hidden sm:flex bg-body p-1 rounded-full w-fit">
              {types.map((type) => (
                <button
                  key={type}
                  onClick={() => setActiveType(type)}
                  className={`px-3.5 py-2.5 rounded-full text-xs transition-all ${activeType === type
          ...
          {showFilters && (
            <div className="sm:hidden mt-4 bg-body p-1 rounded-full flex flex-wrap gap-2">
              {types.map((type) => (
                <button
                  key={type}
                  onClick={() => {
                  ...
                  className={`px-3 py-2 rounded-full text-xs transition-all grow ${activeType === type
                    ? 'bg-primary text-white'
